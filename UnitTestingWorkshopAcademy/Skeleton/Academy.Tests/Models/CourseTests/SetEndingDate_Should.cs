...
namespace Academy.Tests.Models.CourseTests
{
    ...
    public class SetEndingDate_Should
    {
        [DataTestMethod]
        [DataRow(null)]
        public void ThrowArgumentException_WhenInvalidValueIsPassed(DateTime? endingDate)
        {
            // Arange
            var course = new Course("Telerik Alpha .NET", 5, new DateTime(2017, 02, 10), new DateTime(2017, 03, 10));

            // Act & Asserte
            Assert.ThrowsException<ArgumentNullException>(() => course.EndingDate = endingDate);
        }
    }
}
